  ...
  <head>
	...
	<body>
		<div class="con" id="user" style="padding-top: 70px;">
			...
			<div class="warp">
				...
				<div class="table">
					<div class="or clearfix">
						<%if(Authority.haveRigth(user.getId(),"dwgl_wbdw_add")) {%>
						<p class="fl add"    onclick="add();"><img src="<%=path%>/img/add.png" />新增</p>
						<%} %>
						<%if(Authority.haveRigth(user.getId(),"dwgl_wbdw_del")) {%>
            ...
            <%if(Authority.haveRigth(user.getId(),"dwgl_wbdw_upload")) {%>
            ...
            <%} %>
				...
				<div class="table_con">
							...
							<c:forEach items="${list}" var="list" varStatus="s">
								<tr>
									...
									<td>
										...
										<%if(Authority.haveRigth(user.getId(),"dwgl_wbdw_update")) {%>
						...
						<div class="choose">
								<p class="num">当前显示<span><c:if test="${page.pageNum==0}">${(page.pageNum+1)*1 }</c:if><c:if test="${page.pageNum!=0}">${(page.pageNum)*(page.pageSize) }</c:if></span>到<span>${(page.pageNum+1)* (page.pageSize)}</span>条，共<span>${page.count }</span>条记录</p>							<div class="page">
