			...
			<div class="table">
				...
				<div class="table_con" >
					<p class="fill">
						...
						<input  type="hidden"  name="users.id" value="${list.id}">
						<select name="users.type" id="type">
							<option value="" 	id="c1">请选择</option>
							<option <c:if test="${list.type==1}">selected="selected" </c:if>  value="1"  onclick="type('1');"	id="c1">超级管理员</option>
							<option <c:if test="${list.type==2}">selected="selected" </c:if> value="2" onclick="type('2');"		id="c2">区域用户</option>
							<option <c:if test="${list.type==3}">selected="selected" </c:if> value="3"	onclick="type('3');"	 id="c3">终端用户</option>
						</select>
					</p>
					<p class="fill">
					...
					<p class="fill">
						...
						<input type="text" id="logn"  name="users.loginname"  onblur="skip();"  value="${list.loginname}"/>
					</p>
					<p class="fill">
					...
					<p class="fill"  id="quyu"  <c:if test="${list.type!=2 and list.type!=3}"> style="display: none;" </c:if>>
						<label for="quyu" style="display:inline-block ;" >区域单位&nbsp;:&nbsp;</label>
						<input type="hidden"  id="regionUnitId"  name="users.regionUnitId"  value="${list.regionUnitId}"/>
						<input type="text"  id="regionUnitId1"   name="users.unit" value="${list.unit}"  placeholder="请选择" readonly="readonly"   onclick="selectRegionUnitId('regionUnitId','regionUnitId1');"/>
					</p>
					<p class="fill"  id="unitType"  <c:if test="${ list.type!=3}"> style="display: none;" </c:if>>
						<label for="man" style="display:inline-block ;">单位类型&nbsp;:&nbsp;</label>
							<select id="flag" name="users.unitType">
									<option value="">请选择</option>
									<option <c:if test="${list.unitType==1}">selected="selected"</c:if> value="1">使用单位</option>
									<option <c:if test="${list.unitType==2}">selected="selected"</c:if>  value="2">物业单位</option>
									<option <c:if test="${list.unitType==3}">selected="selected"</c:if>  value="3">维保单位</option>
									<option <c:if test="${list.unitType==4}">selected="selected"</c:if>  value="4">制造单位</option>
									<option <c:if test="${list.unitType==5}">selected="selected"</c:if>  value="5">救援单位</option>
							<!-- 		<option value="6">保险单位</option> -->
							</select>
					</p>
					<p class="fill"  id="unitName"  <c:if test="${ list.type!=3}"> style="display: none;" </c:if>>
						<label for="man" style="display:inline-block ;">单位名称&nbsp;:&nbsp;</label>
							<input type="hidden"  id="unitId"   name="users.unitId" value="${list.unitId}"/>
							<input type="text"  id="unitId1"  name="users.unitName"  value="${list.unitName}" readonly="readonly" onclick="selectTypeUnitId('unitId','unitId1','flag');"/>
					</p>
