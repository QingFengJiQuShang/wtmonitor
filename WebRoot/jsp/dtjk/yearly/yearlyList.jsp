		...
		<div class="con" id="user">
			...
			<div class="warp">
				...
				<div class="clearfix">
					...
					<p class="fl">
						<label for="code">检验日期&nbsp;:&nbsp;</label>
						<input type="text" id="time"  value="<fmt:formatDate value='${time}' pattern='yyyy-MM-dd'/>" />
					</p>
					<p class="fl">
						<label for="man">检验单位&nbsp;:&nbsp;</label>
						<input type="text"  id="inspectionUnit"  value="${inspectionUnit}"  placeholder="请输入" readonly="readonly"  />
				...
				<div class="table">
					...
					<div class="or clearfix">
						<%if(Authority.haveRigth(user.getId(),"dtjk_njjl_add")) {%>
						<p class="fl add"    onclick="add('${elevatorId}');"><img src="<%=path%>/img/add.png" />新增</p>
						<%} %>
						<%if(Authority.haveRigth(user.getId(),"dtjk_njjl_del")) {%>
						<p class="fl del">批量删除</p>
						<%} %>
            <%if(Authority.haveRigth(user.getId(),"dtjk_njjl_exp")) {%>
							...
							<c:forEach items="${list}" var="list" varStatus="s">
								<tr>
									...
									<td>
										...
										<%if(Authority.haveRigth(user.getId(),"dtjk_njjl_update")) {%>
