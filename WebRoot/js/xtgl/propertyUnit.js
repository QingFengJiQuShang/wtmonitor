	function gotoUrl (){
	   ...
	   //模糊查询
		function query(){
    		  window.location.href="propertyUnitAction.do?method=query"+gotoUrl();
		  }
		//添加
		function add(){
	...
	JqueryDialog.Open('导入物业单位', 'jsp/xtgl/propertyUnit/upload.jsp', 600, 300);
		...
		//列表分页  
		  function fenye(num){
    		  ...
    		  window.location.href="propertyUnitAction.do?method=query&num="+num+gotoUrl();
         }
		  //编辑
		  function findById(id,flag){
    		  window.location.href="propertyUnitAction.do?method=findById&id="+id+"&flag="+flag;
         }
		//删除
		  function del(id){
			   if( confirm("确定删除这条物业单位记录？")){
    		  		window.location.href="propertyUnitAction.do?method=delEntity&id="+id;
    		  }
         }
		    //下载  
		  function exp(){
    		  window.location.href="propertyUnitAction.do?method=export"+gotoUrl ();
         }
...
$(".del").click(function() {
	...
	$(".wei").each(function() {
		 ...
		 if(len>0 && confirm("确定删除这 "+len+" 条记录？")){
	 		window.location.href="propertyUnitAction.do?method=deleteEntity&ids="+ids;
		 }
})
