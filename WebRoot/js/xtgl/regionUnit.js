	function gotoUrl (){
	   ...
	   //模糊查询
		function query(){
    		  window.location.href="regionUnitAction.do?method=query"+gotoUrl();
		  }
		//添加
		function add(){
			  window.location="jsp/xtgl/regionUnit/addRegionUnit.jsp";
 ...
 */
upload=function(){
	JqueryDialog.Open('导入区域单位', 'jsp/xtgl/regionUnit/upload.jsp', 600, 300);
}
		//列表分页  
		  function fenye(num){
			
    		  window.location.href="regionUnitAction.do?method=query&num="+num+gotoUrl();
         }
		  //编辑
		  function findById(id,flag){
    		  window.location.href="regionUnitAction.do?method=findById&id="+id+"&flag="+flag;
         }
		//删除
		  function del(id){
			   if( confirm("确定删除这条区域单位记录？")){
    		  		window.location.href="regionUnitAction.do?method=delEntity&id="+id;
    		  }
         }
		    //下载  
		  function exp(){
		...
		//列表分页  
		  function fenye1(num){
			 ...
			 var id1= document.getElementById("id1").value;
    		  window.location.href="regionUnitAction.do?method=query1&num="+num+"&id="+id+"&id1="+id1+gotoUrl();
         }
		  	//		批量删除
$(".del").click(function() {
	var ids="";
	...
	$(".wei").each(function() {
		 ...
		 if(len>0 && confirm("确定删除这 "+len+" 条记录？")){
	 		window.location.href="regionUnitAction.do?method=deleteEntity&ids="+ids;
		 }
})
