...
public class XtglRescueUnitDaoImpl  extends BaseDaoImpl< XtglRescueUnit, String> implements  XtglRescueUnitDao{
	...
	public void export(String filePath, XtglRescueUnit unit) {
		try {
			...
			HSSFRow row = null;
			HSSFCell cell = null;
			List<XtglRescueUnit> list=new ArrayList<XtglRescueUnit>();
			...
			//查询服务订单
			String sql="select dr.*  from xtgl_rescue_unit dr where  1=1 " ;
			...
			while(rs.next()){
				XtglRescueUnit elevators=new XtglRescueUnit();
			...
			if (1 == 1) {
				top_arraydis = ExcelColumns.XtglRescueUnit;
				...
				for (int i = 0; i < list.size(); i++) {
					    ...
					    XtglRescueUnit	e = (XtglRescueUnit) list.get(i);
