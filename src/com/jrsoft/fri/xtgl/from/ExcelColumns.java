package com.jrsoft.fri.xtgl.from;

public class ExcelColumns {
	
	public final static String region[] = {"行政区域","客户名称"};
	
	public final static String elevator[] = {"注册号","识别码","电梯品牌","电梯型号","电梯类型(直梯/扶梯)","电梯速度","总层数","注册状态(已注册/未注册)","地图标注(经纬度)","视频ip","维保合同起","维保合同至","省/直辖市","市","市辖区/县","使用单位","使用单位电梯负责人","使用单位电梯负责人电话","制造单位","物业单位","物业单位电梯负责人","物业单位电梯负责人电话","维保单位","维保人","安装单位","安装地点","安装人员","安装时间","生产日期","使用年限","首次检查时间"};
	
	public final static String gateway[] = {"电梯网关类型","取流方式","入网方式","通信线路","终端机型","硬件版本","软件版本","SIM卡号","上报周期","设备序列号","IP地址","port"};
	
	public final static String record[] = {"电梯注册号","日期","时间","电梯方向","电梯楼层","是否有人","电梯门状态","电梯状态"};
	
	public final static String MaintenanceUnit[] = {"单位名称","负责人","负责人电话","办公地点","公司代码","法人","省","市","区"};
	
	public final static String XtglUseUnit[] = {"使用单位名称","联系人","联系人电话","使用单位地址","省","市","区"};
	public final static String XtglSafeUnit[] = {"保险单位名称","联系人","联系人电话","保险单位地址","省","市","区"};
	public final static String XtglRegionUnit[] = {"区域单位名称","联系人","联系人电话","区域单位地址","省","市","区"};
	public final static String XtglPropertyUnit[] = {"物业单位名称","联系人","联系人电话","物业单位地址","省","市","区"};
	public final static String XtglMakeUnit[] = {"制造单位名称","联系人","联系人电话","制造单位地址","省","市","区"};


	public final static String XtglMaintenanceUsers[] = {"维保人","电话","维保证编号","维保证有效期","维保卡号"};
	
	public final static String XtglRescueUnit[] = {"救援单位名称","救援单位类型","联系人","联系人电话","救援单位地址","省","市","区"};

	public final static String DtjkMaintenanceRecords[] = {"电梯注册号","识别码","使用单位","安装地点","维保日期","维保单位","维保人","维保内容","备注"};
	
	public final static String DtjkYearlyInspection[] = {"电梯注册号","识别码","使用单位","安装地点 ","检验日期","检验单位","检验结果","备注"};
	
	public final static String GzclFault[] = {"电梯注册号","识别码","安装地点 ","故障发生时间","报警时间","接警类型","故障问题","救援到达时间","救援成功时间","困人数量","施救单位","处理信息","值班人","状态"};

	public final static String GzlcAlarm[] = {"电梯注册号","识别码","安装地点 ","故障发生时间","报警时间","报警人","报警人电话","故障问题","故障描述"};

	public final static String XtszLog[] = {"操作时间","操作人员","操作内容 "};
	
	public final static String Safe[] = {"电梯总数","未保电梯数","未保电梯率","在保电梯数","在保电梯率","脱保电梯数","脱保电梯率",	"受理理赔次数",	"受理理赔率",	"最高理赔次数",};
	
	public final static String SafeUnit[] = {"保险单位名称","共投保电梯数","受理理赔次数 ","理赔率"};

	public final static String RegionCount[]={"省","市","区","故障数量","人工接警数量","自动接警数量","困人故障数量","困人率","其它故障数量"};
	
	public final static String FaultCount[]={"单位名称","故障类型名称","故障数量","故障发生率"};
	
	public final static String RescueCount[]={"省","市","区","救援次数","平均救援到达时间(分钟)","平均救援成功时间(分钟)"};
	
	public final static String RescueCount1[]={"单位名称","救援次数","平均救援到达时间(分钟)","平均救援成功时间(分钟)"};
	
	public final static String MaintenanceCount[]={"省","市","区","电梯总数","维保刷卡数","维保刷卡率","困人率"};

	public final static String MaintenanceCount1[]={"单位名称","电梯总数","维保刷卡数","维保刷卡率","故障总数","困人故障数量","困人率"};

	public final static String MessageCount[]={"姓名","手机号","短信条数"};


}
